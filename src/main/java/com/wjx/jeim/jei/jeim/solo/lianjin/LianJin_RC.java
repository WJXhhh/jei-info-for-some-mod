package com.wjx.jeim.jei.jeim.solo.lianjin;

import com.wjx.jeim.jei.RecipesCategories;
import com.wjx.jeim.util.Reference;
import mezz.jei.api.IGuiHelper;
import mezz.jei.api.gui.IDrawable;
import mezz.jei.api.gui.IGuiItemStackGroup;
import mezz.jei.api.gui.IRecipeLayout;
import mezz.jei.api.ingredients.IIngredients;
import net.mcreator.solomon.block.BlockLiakmion;
import net.minecraft.client.resources.I18n;
import net.minecraft.item.ItemStack;

import javax.annotation.Nullable;

public class LianJin_RC extends LianJin_ARC<LianJin_R>{
    private final IDrawable background;
    private final IDrawable icon;
    private final String name;

    public LianJin_RC(IGuiHelper helper){
        super(helper);
        background = helper.createDrawable(TEXTURES,4,12,160,100);
        icon = helper.createDrawableIngredient(new ItemStack(BlockLiakmion.block));
        name = I18n.format("jei.recipes.lianjin.name");
    }

    @Override
    public IDrawable getBackground() {
        return background;
    }

    @Override
    public String getTitle() {
        return name;
    }

    @Override
    public String getModName() {
        return Reference.NAME;
    }

    @Override
    public String getUid() {
        return RecipesCategories.Lianjin;
    }

    @Nullable
    @Override
    public IDrawable getIcon() {
        return icon;

    }

    @Override
    public void setRecipe(IRecipeLayout iRecipeLayout, LianJin_R LianJin_R, IIngredients iIngredients) {
        IGuiItemStackGroup stacks = iRecipeLayout.getItemStacks();
        stacks.init(input1,true,56,8); //第二个参数一般来说若是输入端，填true，输出就是false
        stacks.init(input2,true,124,9);
        stacks.init(input3,true,55,48);
        stacks.init(input4,true,123,46);
        stacks.init(input5,true,10,4);
        stacks.init(output,false,89,25);
        stacks.set(iIngredients);
    }
}
